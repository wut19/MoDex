...
from models.dataset.dataset import InhandTransDataset
...
class InhandTrainer:
    ...
    def get_data(self, args):
        # TODO: modify the dataset to include the sequence data
        ...
        self.train_set = InhandTransDataset(data_path=args.train_path, device=self.device)
        self.val_set = InhandTransDataset(data_path=args.val_path, device=self.device)
    ...
    def one_epoch(self, train=True):
        ...
        for i, (data, labels) in enumerate(pbar):
            with torch.autocast("cuda") and (torch.no_grad() if not train else torch.enable_grad()):
                init_hand_state = data[0].to(self.device)
                init_obj_state = data[1].to(self.device)
                actions = data[2].to(self.device)
                next_hand_states = labels[0].to(self.device)
                next_obj_states = labels[1].to(self.device)
                
                preds = []
                state = torch.cat([init_hand_state, init_obj_state], dim=-1)
                next_states = torch.cat([next_hand_states, next_obj_states], dim=-1)
            ...
            if train:
                ...
                unnormalized_hand_preds = preds[..., :15] * self.train_set.hand_std.to(self.device) + self.train_set.hand_mean.to(self.device)
                unnormalized_obj_preds = preds[..., 15:] * self.train_set.obj_std.to(self.device) + self.train_set.obj_mean.to(self.device)
                unnormalized_preds = torch.cat([unnormalized_hand_preds, unnormalized_obj_preds], dim=-1)
                unnormalized_next_hand_states = next_states[..., :15] * self.train_set.hand_std.to(self.device) + self.train_set.hand_mean.to(self.device)
                unnormalized_next_obj_states = next_states[..., 15:] * self.train_set.obj_std.to(self.device) + self.train_set.obj_mean.to(self.device)
                unnormalized_next_states = torch.cat([unnormalized_next_hand_states, unnormalized_next_obj_states], dim=-1)
