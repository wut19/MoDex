...
class ObjHoldFixedEnvV0(BaseV0):
    ...
    DEFAULT_OBS_KEYS = ['tip_pos', 'obj_pos', 'obj_err']
    ...
    def _setup(self,
        ...
        self.observation_space = gym.spaces.Box((-10)*np.ones(60), 10*np.ones(60), dtype=np.float32)
...
class ObjHoldRandomEnvV0(ObjHoldFixedEnvV0):
    ...
    # def reset(self, pos=None, size=None, **kwargs):
    #     # randomize target pos
    #     if pos is None:
    #         self.target_pos = self.object_init_pos + self.np_random.uniform(high=np.array([0.030, 0.030, 0.030]), low=np.array([-.030, -.030, -.030]))
    #     else:
    #         self.target_pos = pos
    #     self.sim.model.site_pos[self.goal_sid] = self.target_pos
    #     # randomize object
    #     if size is None:
    #         self.size = self.np_random.uniform(high=np.array([0.030, 0.030, 0.030]), low=np.array([.020, .020, .020]))
    #     else:
    #         self.size = size
    #     self.sim.model.geom_size[-1] = self.size
    #     self.sim.model.site_size[self.goal_sid] = self.size
