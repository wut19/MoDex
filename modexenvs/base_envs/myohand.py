...
from myosuite.envs.myo.myobase import Rpos

import os
import cv2

class MyoHand(BaseV0):

    DEFAULT_OBS_KEYS = ['qpos', 'qvel', 'tip_pose', 'pose_err']
    ...
    model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../assets', 'mjcf/myosuite/myohand.xml')

    def __init__(self, model_path=model_path, obsd_model_path=None, seed=None):
        kwargs={
    ...
    def _setup(self,
        ...
        self.viewer_setup(azimuth=180, distance=0., lookat=[-0.15,-0.5,1.3],render_actuator=True)
    ...
    def get_obs_vec(self):
        ...
        self.obs_dict['tip_pose'] = np.array([])
        for isite in range(len(self.tip_sids)):
            self.obs_dict['tip_pose'] = np.append(self.obs_dict['tip_pose'], self.sim.data.site_xpos[self.tip_sids[isite]].copy())
            self.obs_dict['tip_pose'] = np.append(self.obs_dict['tip_pose'], self.sim.data.site_xmat[self.tip_sids[isite]].copy())
    ...
    def get_obs_dict(self, sim):
        ...
        obs_dict['tip_pose'] = np.array([])
        for isite in range(len(self.tip_sids)):
            obs_dict['tip_pose'] = np.append(obs_dict['tip_pose'], self.sim.data.site_xpos[self.tip_sids[isite]].copy())
            obs_dict['tip_pose'] = np.append(obs_dict['tip_pose'], self.sim.data.site_xmat[self.tip_sids[isite]].copy())
        return obs_dict
    ...
    def render_offscreen(self, width=640, height=480):
        rbg = self.sim.renderer.render_offscreen(width=width, height=height, camera_id=4)
